...
async function fetchHistoricalWeather(lat, lng, date, label) {
  ...
  const base = daysAgo <= 7 ? `https://api.open-meteo.com/v1/forecast` : `https://archive-api.open-meteo.com/v1/archive`;
  const url =
    `${base}` +
    `?latitude=${lat}&longitude=${lng}` +
  ...
  const current = ref !== -1 ? { temp: Math.round(wx.hourly.temperature_2m[ref]), code: wx.hourly.weather_code[ref] } : { temp: null, code: null };
